...
if (isset($_POST['btnModificarImages'])) {
    if (empty($_POST["txtId"]) || empty($_POST["Product"])  || empty($_FILES["txtImagen1"])  || empty($_FILES["txtImagen2"]) || empty($_FILES["txtImagen3"]) || empty($_FILES["txtImagen4"]) ||  empty($_FILES["txtImagen5"])) {
        header("location:  ../pages/editimages.php?txtEmptyError");
        return;
    }
    $image->updateImagen($_POST["txtId"], $_POST["Product"], $_FILES["txtImagen1"], $_FILES["txtImagen2"], $_FILES["txtImagen3"], $_FILES["txtImagen4"], $_FILES["txtImagen5"]);
}
